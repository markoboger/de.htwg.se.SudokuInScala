...
import de.htwg.se.sudoku.model.gridComponent.gridBaseImpl.{
  GridCreateRandomStrategy,
  GridCreateStrategyTemplate,
  Solver,
  Grid => BaseGrid
}
...
class Grid @Inject()(@Named("DefaultSize") size: Int) extends BaseGrid(size) {
  ...
  override def createNewGrid: GridInterface =
    (new GridCreateSymmetricStrategy).createNewGrid(size)

}

class GridCreateSymmetricStrategy extends GridCreateStrategyTemplate {

  override def prepare(grid: GridInterface): GridInterface =
    (new GridCreateRandomStrategy).createNewGrid(grid.size).solve._2

  override def fill(grid: GridInterface): GridInterface =
    removeSymmetricPairs(grid, remove = grid.size * grid.size / 4)
  ...
  def removePair(grid: GridInterface,
                 row1: Int,
                 col1: Int,
                 row2: Int,
                 col2: Int): GridInterface = {
    grid.reset(row1, col1).reset(row2, col2)
  }

  def filledCells(grid: GridInterface): immutable.IndexedSeq[(Int, Int)] = {
    for (row: Int <- 0 until grid.size; col: Int <- 0 until grid.size;
         if (grid.cell(row, col).isSet)) yield (row, col)
  }

  def numSetCells(grid: GridInterface): Int = filledCells(grid).length

  def randomSetCell(grid: GridInterface): (Int, Int) =
    filledCells(grid)(scala.util.Random.nextInt(numSetCells(grid)))

  def symmetricCell(size: Int, row: Int, col: Int): (Int, Int) =
