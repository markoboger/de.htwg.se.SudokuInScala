...
case class Grid(cells: Matrix[Cell]) extends GridInterface {
  ...
  def reset(row: Int, col: Int): Grid =
    copy(cells.replaceCell(row, col, Cell(0, false, false, false)))

  def setGiven(row: Int, col: Int, value: Int): Grid =
    copy(cells.replaceCell(row, col, Cell(value, given = true)))
  ...
  def highlight(index: Int): Grid = {
    ...
    for {
      row <- 0 until size
      col <- 0 until size
    } if (available(row, col).contains(index))
      grid = grid.setHighlighted(row, col)
    else grid = grid.unsetHighlighted(row, col)
    grid
  }

  def setHighlighted(row: Int, col: Int): Grid =
    copy(cells.replaceCell(row, col, cell(row, col).copy(isHighlighted = true)))

  def unsetHighlighted(row: Int, col: Int): Grid =
    copy(
      cells.replaceCell(row, col, cell(row, col).copy(isHighlighted = false)))

  def isHighlighted(row: Int, col: Int) = cell(row, col).isHighlighted

  def setShowCandidates(row: Int, col: Int): Grid =
    copy(
      cells.replaceCell(row, col, cell(row, col).copy(showCandidates = true)))

  def isShowCandidates(row: Int, col: Int) = cell(row, col).showCandidates

  def unsetShowCandidates(row: Int, col: Int): Grid =
    copy(
      cells.replaceCell(row, col, cell(row, col).copy(showCandidates = false)))
  ...
  def blockAt(row: Int, col: Int): Int =
    (col / blocknum) + (row / blocknum) * blocknum
  ...
  def valid: Boolean =
    allrows.forall(house => house.valid) && allcols.forall(house => house.valid) && allblocks
      .forall(house => house.valid)

  def isSymmetric: Boolean = {
    val resultList: IndexedSeq[Boolean] =
      for (row <- 0 until size; col <- 0 until size) yield {
        if ((cell(row, col).isSet == true && symmetricCell(row, col).isSet == true) || (cell(
              row,
              col).isSet == false && symmetricCell(row, col).isSet == false))
          true
        else false
      }
  ...
  def symmetricCell(row: Int, col: Int): Cell =
    cell(size - 1 - row, size - 1 - col)

  def available(row: Int, col: Int): Set[Int] =
    if (cell(row, col).isSet) {
      Set.empty
    } else {
      (1 to size).toSet -- rows(row).toIntSet -- cols(col).toIntSet -- blocks(
        blockAt(row, col)).toIntSet
    }

  def solved: Boolean =
    cells.rows.forall(coll => coll.forall(cell => cell.isSet))
  ...
  override def createNewGrid: GridInterface =
    (new GridCreateRandomStrategy).createNewGrid(size)
